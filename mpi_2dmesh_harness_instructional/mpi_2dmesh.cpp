...
#endif
                off_t s_offset=0, pad_offset=0;
                vector <float> padS((global_width+2) * (global_height+2), 0);
                float *padPtr = padS.data() + (global_width+2) + 1;
                for (int i=0; i<global_height; i++) {
                    memcpy((void *)(padPtr + pad_offset), (void *)(s+s_offset), sizeof(float)*global_width);
                    pad_offset += global_width+2;
